...
export default function TiptapEditor({
  ...
  if (!editor) {
    return (
      <div className="border-2 border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm min-h-[280px] flex items-center justify-center">
  ...
  return (
    <div className="border-2 border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow duration-300">
      {/* Toolbar */}
      <div className="flex flex-wrap items-center gap-1 p-3 bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
        ...
        <div className="relative" ref={fontDropdownRef}>
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white border border-slate-200 bg-white text-sm font-medium text-slate-700 min-w-[160px] justify-between transition-all duration-200 hover:border-indigo-300 hover:shadow-sm"
          >
            <Type size={16} className="text-indigo-500 flex-shrink-0" />
            <span className="truncate flex-1 text-left">{currentFont}</span>
            <ChevronDown 
              size={14} 
              className={`text-slate-400 flex-shrink-0 transition-transform duration-200 ${fontDropdownOpen ? 'rotate-180' : ''}`} 
            />
          </button>
          
          {fontDropdownOpen && (
            <div className="absolute top-full left-0 mt-1 w-56 max-h-72 overflow-y-auto bg-white border border-slate-200 rounded-xl shadow-xl z-50">
              {FONTS.map((font, index) => (
                <button
                  ...
                  } ${index === FONTS.length - 1 ? 'rounded-b-xl' : ''} ${
                    currentFont === font.name ? 'bg-indigo-50 text-indigo-700' : ''
                  }`}
                >
                  {font.name}
                </button>
        ...
        <div className="relative" ref={sizeDropdownRef}>
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white border border-slate-200 bg-white text-sm font-medium text-slate-700 min-w-[80px] justify-between transition-all duration-200 hover:border-indigo-300 hover:shadow-sm"
          >
            <span>{currentSize}</span>
            <ChevronDown 
              size={14} 
              className={`text-slate-400 flex-shrink-0 transition-transform duration-200 ${sizeDropdownOpen ? 'rotate-180' : ''}`} 
            />
          </button>
          
          {sizeDropdownOpen && (
            <div className="absolute top-full left-0 mt-1 w-20 max-h-72 overflow-y-auto bg-white border border-slate-200 rounded-xl shadow-xl z-50">
              {FONT_SIZES.map((size, index) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 hover:bg-indigo-50 text-sm text-slate-700 transition-colors ${
                    index === 0 ? 'rounded-t-xl' : ''
                  } ${index === FONT_SIZES.length - 1 ? 'rounded-b-xl' : ''} ${
                    currentSize === size.label ? 'bg-indigo-50 text-indigo-700' : ''
                  }`}
                >
                  {size.label}
                </button>
        ...
        <button
          ...
          className={`p-2.5 rounded-lg transition-all duration-200 ${
            editor.isActive('bold')
              ...
              : 'hover:bg-white text-slate-600 hover:text-indigo-600 border border-transparent hover:border-slate-200'
          }`}
          title="Bold (Ctrl+B)"
        >
          <Bold size={18} />
